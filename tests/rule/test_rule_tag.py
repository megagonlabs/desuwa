...
class TestRuler(unittest.TestCase):
    ...
    def test_rule(self):
        ...
        for idx, (gold, center_gold, next_gold) in enumerate(zip(golds, center_golds, next_golds)):
            self.assertEqual(None, r.prev_constraints)

            self.assertEqual(center_gold, r.constraints.match(tags, idx, idx, idx))
            if idx != len(tags) - 1:
                _m = r.next_constraints.match(tags, idx + 1, idx + 1, len(tags) - 1)
